...
#define ALPHA 1.0
#define BETA 4.0 //This parameter raises the weight of distance over pheromone
#define RHO 0.3 //Evapouration rate
#define QVAL 100
...
int selectNextCity( int ant ) {
    ...
    do {
        ...
        if(to >= MAX_CITIES)
            to=0;
        if(ants[ant].tabu[to] == 0) { // time constraints here
...
start_ = clock();
    ...
    cout<<"\nSACO: Best tour = "<<best<<endl<<endl<<endl;
     for(int i=0; i<MAX_CITIES; i++) cout << ants[bestIndex].path[i]<< " ";
    emitDataFile(bestIndex);
    end_ = clock();
    CTU = double(end_ - start_)/CLOCKS_PER_SEC;
   printf("%.2f",CTU);
    return 0;
}
